...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-explain
...
# ATTACH (`${VGI_EXPLAIN_WORKER}`):
#
#   subprocess : a bare stdio command (`.venv/bin/vgi-explain`) — the extension
...
#   VGI_EXPLAIN_WORKER   the stdio command that runs the worker (the installed
#                        `vgi-explain` console script in the project venv). Used
#                        directly as the LOCATION for subprocess, and as the
#                        process this script boots for http/unix.
# Optional:
...
: "${HAYBARN_UNITTEST:?path to the haybarn-unittest binary}"
: "${VGI_EXPLAIN_WORKER:?worker LOCATION (stdio command running the worker)}"
...
# VGI_EXPLAIN_WORKER with a URL.
WORKER_CMD="$VGI_EXPLAIN_WORKER"
...
# The .test files read committed parquet fixtures by relative path
# (test/fixtures/*.parquet) via DuckDB's read_parquet, which resolves them
# relative to the runner's cwd. The runner cd's into the stage dir, so copy them
# in. (For http/unix we also boot the worker with cwd = the stage dir below, so
# any relative path the worker itself resolves matches.)
cp -R "$REPO/test/fixtures" "$STAGE/test/fixtures"

# ---------------------------------------------------------------------------
# Per-transport: resolve VGI_EXPLAIN_WORKER (the LOCATION) and, for the
...
case "$TRANSPORT" in
  subprocess)
    echo "Transport: subprocess/stdio — VGI_EXPLAIN_WORKER=$VGI_EXPLAIN_WORKER"
    ;;
  ...
  http)
    ...
    # (waitress); the integration job installs it via `uv sync --extra http`.
    # cwd = STAGE so any relative path the worker resolves matches the runner.
    PORT_FILE="$(mktemp -u "${TMPDIR:-/tmp}/exp-port.XXXXXX")"
    LOG_FILE="${TMPDIR:-/tmp}/exp-http-server.log"
    ...
    export VGI_EXPLAIN_WORKER="http://127.0.0.1:$PORT"
    echo "HTTP worker ready on $VGI_EXPLAIN_WORKER (pid $SERVER_PID)"
    ;;
  ...
  unix)
    ...
    SOCK="${TMPDIR:-/tmp}/exp-$$.sock"
    rm -f "$SOCK"
    LOG_FILE="${TMPDIR:-/tmp}/exp-unix-server.log"
    ...
    export VGI_EXPLAIN_WORKER="unix://$SOCK"
    echo "unix worker ready on $VGI_EXPLAIN_WORKER (pid $SERVER_PID)"
    ;;
esac
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_EXPLAIN_WORKER) ..."
