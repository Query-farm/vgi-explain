...
#   VGI_EXPLAIN_WORKER   worker LOCATION the .test files ATTACH (a stdio command
#                        such as `.venv/bin/vgi-explain`, or an http:// URL)
# Optional:
#   STAGE                scratch dir for the preprocessed test tree (default: mktemp)
set -euo pipefail

: "${HAYBARN_UNITTEST:?path to the haybarn-unittest binary}"
: "${VGI_EXPLAIN_WORKER:?worker LOCATION (stdio command or http:// URL)}"
...
# (test/fixtures/*.parquet). The runner cd's into the stage dir, so copy them in.
...
echo "Running suite (worker: $VGI_EXPLAIN_WORKER) ..."
