"""In-process VGI invocation helpers for the explain worker test suite.

Drives a *source* table function through the real bind -> init -> process
lifecycle without spawning a worker process, so those tests stay fast. The
streaming (``shap_values``) and buffering (``feature_importance``) functions are
exercised end-to-end against a real ``vgi.client.Client`` in test_client.py,
which is the faithful way to drive their input/finalize phases.
"""
